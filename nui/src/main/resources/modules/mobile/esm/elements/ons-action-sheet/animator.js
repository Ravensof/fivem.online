...
export var ActionSheetAnimator = function (_BaseAnimator) {
	...
	function ActionSheetAnimator() {
		...
		return _possibleConstructorReturn(this, (ActionSheetAnimator.__proto__ || _Object$getPrototypeOf(ActionSheetAnimator)).call(this, {
			timing: timing,
			delay: delay,
			duration: duration
		}));
...
export var MDActionSheetAnimator = function (_ActionSheetAnimator) {
	...
	function MDActionSheetAnimator() {
		...
		var _this2 = _possibleConstructorReturn(this, (MDActionSheetAnimator.__proto__ || _Object$getPrototypeOf(MDActionSheetAnimator)).call(this, {
			timing: timing,
			delay: delay,
			duration: duration
		}));
	...
	_createClass(MDActionSheetAnimator, [{
		...
		value: function hide(dialog, callback) {
			animit.runAll(animit(dialog._mask).queue({opacity: 1}).wait(this.delay).queue({opacity: 0}, {
				...
				opacity: 1
			}, {transform: 'translate3d(0, 80%, 0)', opacity: 0}).queue(function (done) {
				callback && callback();
				done();
			}));
		}
...
export var IOSActionSheetAnimator = function (_ActionSheetAnimator2) {
	...
	function IOSActionSheetAnimator() {
		...
		var _this3 = _possibleConstructorReturn(this, (IOSActionSheetAnimator.__proto__ || _Object$getPrototypeOf(IOSActionSheetAnimator)).call(this, {
			timing: timing,
			delay: delay,
			duration: duration
		}));
