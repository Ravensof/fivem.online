...
var LiftToastAnimator = function (_ToastAnimator) {
	...
	function LiftToastAnimator() {
		...
		var _this = _possibleConstructorReturn(this, (LiftToastAnimator.__proto__ || _Object$getPrototypeOf(LiftToastAnimator)).call(this, {
			timing: timing,
			delay: delay,
			duration: duration
		}));
	...
	_createClass(LiftToastAnimator, [{
		...
		value: function hide(toast, callback) {
			...
			animit.runAll(animit(toast, this.def).default({
				...
				opacity: 1
			}, {transform: 'translate3d(0, ' + this.liftAmount + ', 0)', opacity: 0}).queue(function (done) {
				callback && callback();
				done();
			}));
		}
